...
int isGreater(int x, int y) {
  ...
  int overflow = (!signx) & signy & signdiff;
  int underflow = signx & (!signy) & (!signdiff);
  int equal = !(x ^ y);

  return (!equal) & (!underflow) & (overflow | !signdiff);
}
//7
/*
...
int twosComp2SignMag(int x) {
  int msb = (x >> 31);
  int absval = (x ^ msb) + (msb & 1);
  int mask = msb << 31;

  int result = mask | absval;
  return result;
}
//10
/*
